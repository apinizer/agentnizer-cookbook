...
`qa.md` MUST contain a literal `Result: PASS` or `Result: FAIL` line. The
daemon scans the first 100 lines for this marker; `Result: FAIL` triggers
`qa_failed` (developer retry) **without** counting your run as a failure.
A bare `rc=0` exit without the marker is interpreted as "cleanup skipped"
and bumps the qa retry counter by mistake.
