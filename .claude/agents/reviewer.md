...
#### Severity-based fast-fail (correctness / convention / quality)

Within a sub-reviewer's verdict, the call is:
- **CRITICAL ≥ 1**  → sub-review FAIL  (unconditional)
- **HIGH ≥ 3**      → sub-review FAIL  ("MAJOR" in older docs is a synonym for HIGH)
- otherwise         → sub-review PASS (MEDIUM / LOW findings surfaced as
  non-blocking notes for the developer's next pass)

Apply this when reading each sub-reviewer's `findings` list. The rule
prevents borderline diffs from sliding through with two HIGHs ignored.
