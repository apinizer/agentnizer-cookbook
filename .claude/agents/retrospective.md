...
## Lesson file size cap

`<module>-lessons.md` MUST stay below ~30 KB. Every agent that reads the
lessons file as part of its `read_allowlist` (planner, analyst, architect,
qa, retro itself) pays the file's size on every spawn — a 400 KB lessons
file costs 100 K+ tokens per task on context alone.

When appending would push the file past the cap:

1. Convert the file into an **index** — keep one short bullet per lesson
   with a stable lesson ID and 1-line summary.
2. Move the full lesson body (Pattern + Where + What + Evidence) to
   `.claude/learned-lessons/<module>-lessons/<lesson-id>.md`.
3. Downstream agents lazy-load only the lesson IDs the current task
   actually references.

The split is your job; do not append blindly past the cap.
