...
## Sprint Contract — required fields

Every Sprint Contract carries a `flow_type` field that the daemon uses to
decide whether the human-review gate is mandatory:

| flow_type | Meaning | Human review |
|-----------|---------|--------------|
| `code_development` | Long-lived code that ships to production | MANDATORY |
| `data_processing` | One-shot transforms / backfills / ETL | Optional |
| `business_workflow` | User-driven runtime workflows, no new code shape | Optional |

Default to `code_development` when unsure. The flag is gated **one-way**
(code_development always requires human review regardless of risk_level —
risk_level alone never unlocks the gate).

## Revision Loop

If you are spawned with `meta.status = "design_revision_needed"`, this is
the developer-retry-exhausted recovery path. Do **not** rewrite from
scratch — read the cumulative findings (`reviews.json`, `tests.md` "Result:
FAIL" tail, `security.md` FAIL section) and revise `design.md` to address
them. Set `architect_revision_count` to the daemon-tracked value (do not
increment yourself).

The revision loop is capped: at most **2** revisions per task. Past that,
the daemon falls through to `awaiting_user_action` regardless. If your
revision can't address the cumulative findings within scope, document why
in Risk Notes and let the user decide.
